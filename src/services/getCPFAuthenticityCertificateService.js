...
const getCPFAuthenticityCertificateService = async (numero, codigoControle, dataEmissao, horaEmissao, tipoCertidao) => {
	...
	try {
		...
		await page.waitForSelector(selectors.authenticityCPFBtn, { delay: 5000 });
		await page.click(selectors.authenticityCPFBtn);
		
		console.log(`[LOG INFO] - ${numero}: Aguardando próxima página.`);
		
		const secondPage = await browser.waitForTarget(target => target.url() === "https://solucoes.receita.fazenda.gov.br/Servicos/certidaointernet/PF/AutenticidadePGFN/Confirmar");
		...
		console.log(`[LOG INFO] - ${numero}: Digitando CPF.`);
...
export default getCPFAuthenticityCertificateService;